 ...
 *
 * ---------------------------------------------------------------------------
 * v2.0.0 - REMOVED THE DELAY-JS EXCLUSION FROM THIS FILE
 * ---------------------------------------------------------------------------
 * An earlier version of this file also excluded jQuery, client_plugins.min.js
 * and client_frontend.min.js from WP Rocket's "Delay JavaScript Execution"
 * queue, to work around a jQuery-placeholder timing issue and an async/defer
 * ordering bug. That exclusion had a real side effect that wasn't discovered
 * until much later: it made Live Composer's dslc_carousel() run immediately
 * on every page load instead of only after the visitor's first interaction.
 * Every .dslc-carousel module on the page initializes through its own
 * independent, asynchronous container.imagesLoaded() callback - forcing
 * several of them to start that race at page-load time instead of being
 * naturally staggered by "wait for interaction" made it possible for two
 * carousels' init sequences to collide, which manifested as one section's
 * content visibly appearing inside another section on the live site.
 *
 * The two problems that exclusion was working around are now both fixed by
 * other means: the async/defer ordering bug is fixed directly (a separate
 * mu-plugin strips a stray `async` attribute from Live Composer's own script
 * tag), and the stub below - an Object.defineProperty trap - is immune to
 * jQuery-placeholder timing regardless of when or how jQuery actually loads,
 * delayed or not. So the exclusion was solving already-solved problems while
 * quietly causing this one. Removing it lets WP Rocket delay-gate jQuery and
 * Live Composer's scripts exactly as it does everywhere else on the site,
 * and Live Composer's carousels go back to initializing once, on the
 * visitor's first interaction, the way they were always designed to.
 *
 * WHAT THIS FILE DOES NOW
 * 1. Prints a tiny stub in <head> that defines jQuery.fn.mediaelementplayer as
 *    a harmless no-op via an Object.defineProperty trap on window.jQuery - it
 *    patches whatever is already there immediately AND keeps listening, so it
 *    also catches a placeholder later being replaced by the real library, with
 *    no polling interval and therefore no race window, regardless of whether
 *    jQuery loads immediately or is delayed until interaction. If a real video
 *    is later loaded on the page, MediaElement.js overwrites this no-op with
 ...
 *    protecting your PageSpeed Insights score. This part never touched Delay JS
 *    timing and is unaffected by the change above.
